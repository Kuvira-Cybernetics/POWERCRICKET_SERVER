...
## Match-end & REST status — known traps (2026-06-16)

**`endMatch` is guarded against re-entry.** The room is kept alive ~60s after a natural end for rematch; a client socket drop in that window re-enters `endMatch` via the PvP `allowReconnection().catch` path (`onLeave`) → `applyRewardsToProfile` would run a SECOND time and double-apply `mmr`/coins/xp/trophies (possibly crediting the opposite winner). First line of `endMatch` is now `if (this.state.phase === "result") return;` (logs `####_PVP_ENDMATCH_REENTRY_BLOCKED`). **Rule:** any terminal handler reachable from a lifecycle callback (reconnection grace, rematch, disconnect) must guard its terminal state — don't assume it runs once. (`multiplayer_audit.md` P1-2.)

**REST endpoints in `routes/api.ts` are partly mock stubs — do NOT treat them as the source of truth.** Durable player data (`mmr`/coins/trophies) is written by `MatchRoom.applyRewardsToProfile` to Firestore `players`, and the Unity client reads Firestore directly. `/leaderboard/global` + `/leaderboard/weekly` are now Firestore-backed (`players` ordered by `mmr`), but `/profile/me`, `/profile/:id/stats`, and `/matchmaking/elo-bracket` still read the in-memory `users` Map and disagree with Firestore. Back them with the Admin SDK or remove before relying on them. (`multiplayer_audit.md` P2-2/P2-5.)
